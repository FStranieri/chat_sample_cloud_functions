...
module.exports = class CloudDBZoneWrapper {
    ...
    async deleteMessage(fullMessageData) {
        if (!this.cloudDBZoneClient) {
            this.logger.info("CloudDBClient is null, try re-initialize it");
            return;
        }
        try {
            const resp = await this.cloudDBZoneClient.executeDelete(fullMessageData);
    ...
    async deleteAllMessages() {
        ...
        try {
            const resp = await this.cloudDBZoneClient.executeDeleteAll(UserMessage.input_messages);
    ...
    async deletePollLunchChoices() {
        if (!this.cloudDBZoneClient) {
            this.logger.info("CloudDBClient is null, try re-initialize it");
            return;
        }
        try {
            const resp = await this.cloudDBZoneClient.executeDeleteAll(PollLunch.poll_lunch);
            this.logger.info('The number of delete all poll lunch choices is:' + resp);
        } catch (error) {
            this.logger.error('deleteAllPollLunchChoices=>' + error);
        }
    }
    ...
    async getAllMessages() {
        ...
        try {
            const resp = await this.cloudDBZoneClient.executeQuery(clouddb.CloudDBZoneQuery.where(UserMessage.input_messages));
